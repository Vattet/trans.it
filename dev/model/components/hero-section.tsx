    ...
    <section className="py-20 md:py-32 px-4 fade-in">
      <div className="max-w-4xl mx-auto text-center">
        <div className="inline-block px-3 py-1 bg-secondary/50 border border-border rounded-full mb-6 slide-up">
          <span className="text-sm font-medium">{t("heroTitle", language)}</span>
        </div>

        <h1 className="text-4xl md:text-6xl font-bold mb-6 text-balance slide-up">
          {t("heroSubtitle", language).split(",")[0]}{" "}
          <span className="text-primary">{t("heroSubtitle", language).split(",")[1]}</span>
        </h1>
        ...
        <div className="grid grid-cols-3 gap-4 max-w-md mx-auto slide-up">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t("maxFileSize", language)}</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-primary">256-bit</p>
            <p className="text-xs text-muted-foreground">{t("encryption", language)}</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-primary">7 {t("oneDay", language).split(" ")[0]}</p>
            <p className="text-xs text-muted-foreground">{t("fileRetention", language)}</p>
          </div>
        </div>
      </div>
    </section>
